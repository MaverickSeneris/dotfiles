...
# 🕒 Minimal Dynamic Greeting
hourmin=$(date +"%H:%M")
echo -e "It's $hourmin, run \e[1;33mbp list\e[0m or \e[1;33mcmd-list\e[0m."
...
alias home='cd ~ && clear && pwd && line && ls --group-directories-first --color=auto -F'
# 🧱 go to root directory
alias root='cd / && clear && pwd && line && ls --group-directories-first --color=auto -F'


# 🔧 Utility function for a colored separator line
line() {
  printf "\e[1;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\e[0m\n"
}
# 🧱 go to Projects directory
alias projects='cd ~/Projects/ && clear && pwd && line && ls -la'
# ⚙️ go to dotfiles directory
alias dotfiles='cd ~/Projects/dotfiles/ && clear && pwd && line && ls -la'
# 🎛️ go to waybar directory
alias wc='cd ~/.config/waybar && clear && pwd && line && ls -la'
# 🧩 go to hyprland directory
alias hc='cd ~/.config/hypr/ && clear && pwd && line && ls -la'
# 🧠 list down all Omarchy terminal commands
alias otp='cd ~/.local/share/omarchy/bin && clear && pwd && line && ls -la'
# edit bashrc prompts
...
# Edit config
# edit .config files
alias ec='nvim ~/.config' # edit .config dotfiles
# edit .local files
alias el='nvim ~/.local' # edit .local dotfiles
# Show all commands ever typed
alias history='cat ~/.bash_history' # list command history
# 'cpp' shows progress when copying files
alias cpp='rsync -ah --progress' # copy files with progress interfaceal
# hyprland bindings
alias binds='cat ~/.config/hypr/bindings.conf' # See keybindings
...
alias pinstall='sudo pacman -S' # install arch packages: ainstall <package-name>
alias pacq='pacman -Ss' # search arch packages: pacq <package-name>
# Yay install and query (AUR)
alias ainstall='yay -S' # install AUR packages: ainstall <package-name>
alias yayq='yay -Ss' # Search AUR packages: yayq <package-name>
...
alias gc='function _gclone(){ git clone git@github.com:$1/$2.git; }; _gclone' # Clone a repo from github
# gpush [message] → add, commit, and push (defaults to "update" if no message)
alias gp='function _gpush(){ msg=${1:-update}; git add . && git commit -m "$msg" && git push; }; _gpush' # Push local repo to remote github repo

#######################
## HARDWARE PROMPTS ##
######################
alias kbu="brightnessctl -d smc::kbd_backlight s 10%+" # increase keyboard lights
alias kbd="brightnessctl -d smc::kbd_backlight s 10%-" # decrease keyboar lights

################################
## SYSTEM MAINTENANCE PRPOMTS ##
################################
# Run rkhunter: update + full scan no skip
alias vs='sudo rkhunter --update && sudo rkhunter --propupd && sudo rkhunter --check --sk --nocolors --report-warnings-only' # Scan system for malwares
...
familydrive() {
    ...
    if [ -d "$DRIVE" ]; then
        ...
        # Header
        printf "\e[1;36m==================== SENERIS FAMILY DRIVE ====================\e[0m\n"
        printf "\e[1;33m📦 Current Directory:\e[0m\n"

        pwd
        printf "\e[1;36m--------------------------------------------------------------\e[0m\n"

        printf "\e[1;33m📂 Contents:\e[0m\n"
        ls -lah

        printf "\e[1;36m==============================================================\e[0m\n"
        echo "✅ Drive mounted and accessed successfully. Steward of data, carry on."
    else
        echo "⚠️  Seneris1TB not found. Plug the sacred drive, noble keeper."
    fi
}
alias familydrive=familydrive


exitdrive() {
    cd "$HOME" && clear
    printf "\e[1;32m🏡 Returned Home\e[0m\n"
    printf "\e[1;36m--------------------------------------------------------------\e[0m\n"
    pwd
    printf "\e[1;36m--------------------------------------------------------------\e[0m\n"
    ls
